...
> R: Carrito_init recibe un puntero a un tipo de dato carrito y lo inicializa en 0. Es importante inicializarla ya que una variable no inicializada puede contener un valor indeterminado.
...
> R: [total: un producto, cantidad 1]  [OK]   carrito_total(&c) == 350
...
> R:[total: un producto, cantidad 2] [FAIL] carrito_total(&c) => esperado 700, obtenido 350  (linea 50).
...
> R: El código dice que el error esta en la linea 50. En esta linea hay un assert que valida que los valores sean iguales, se invoca a carrito_total para verificar el monto del carrito.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: La funcion carrito_total deberia sumar los totales del carrito y solo suma 1 de cada producto. Se debe multiplicar por la cantidad antes de hacer la suma. 
...
> R:[Carrito lleno: MAX_ITEMS = 4] [FAIL] carrito_agregar(&c, p) => esperado 0, obtenido 1  (linea 68)
...
> R: El operador deberia ser "<" en vez de "<=" dado que el array arranca en 0  
...
> R:29:int carrito_descuento(int total, int porcentaje) 30:    return total - (total * porcentaje / 100); Estas lineas no se ejecutan dado que cobertura utiliza el programa test_unitarios y en el mismo no se utilizo la funcion carrito_descuento.
...
> R:Test unitario evalua las funciones por separado mientra que integracion evalua el funcionamiento general del programa. El test unitario detecto el bug dado que se evaluo la funcion puntual.
...
> R:C no comprueba automaticamente los limites de un array, provoca un comportamiento indefinido, en cambio java y python validan y reportan el error.
...
> R: Nos permite verificar el funcionamiento de las funciones e ir desglosando el código por partes de forma automatica sin tener que hacer el seguimiento paso a paso con la herramienta de debug. Con ella podemos ir analizando el código en cada avance y tambien le permite a cualquier programador validar el funcionamiento sin tener que haber sido el quien escribio el código.
...
> R: En el primer test se evalua que el producto se haya cargado mientras que en el segundo test evalua las cantidades cargadas.
